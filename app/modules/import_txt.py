...
+
+TXT_PAT_RA = re.compile(r"\n(RA\d{8})\s+([^\n]+)")
+
+def _read_any(fp):
+    for enc in ("utf-8","latin-1","cp1252"):
+        try:
+            return open(fp, "r", encoding=enc).read()
+        except UnicodeDecodeError:
+            continue
+    return open(fp, "rb").read().decode("latin-1","ignore")
+
+def parse_puc_txt(fp:str):
+    """
+    Extrai: turma, disciplina (se vier no cabeçalho), professor, cod_professor
+    e lista [(ra,name)]
+    """
+    txt = _read_any(fp).replace("\r\n","\n")
+    turma = None
+    # exemplos habituais: "ECO-MA6 Ciências Econômicas"
+    m_turma = re.search(r"\n(ECO-[A-Z0-9]+)\s+Ciências Econômicas", txt)
+    if m_turma:
...
+    m_cod = re.search(r"Cód\.Usuário:\s+(\d+)", txt)
+    cod_prof = m_cod.group(1) if m_cod else ""
+    disciplina = "ECONOMIA INDUSTRIAL" if "ECONOMIA INDUSTRIAL" in txt else ("EBC II" if "ECONOMIA BRASILEIRA" in txt.upper() else "")
+
+    students = []
+    for m in TXT_PAT_RA.finditer(txt):
+        ra = m.group(1).strip()
+        name = re.sub(r"\s{2,}", " ", m.group(2).strip())
+        students.append((ra, name))
+    return {
+        "turma": turma, "disciplina": disciplina, "professor": prof, "cod_professor": cod_prof,
+        "students": students
+    }
+
+def upsert_students_and_enroll(engine, term:str, disciplina_code:str, turma:str, students:list):
+    """
+    - garante offering(term, turma, disciplina)
+    - upsert students por RA
+    - insere/reativa enrollment (active=1)
+    """
